...
void ATEMbase::_sendPacketBuffer(uint8_t length)	{
	...
	for (unsigned int i = 0; i < sizeof(_packetBuffer); i++) {
		Serial.print(_packetBuffer[i]);
	}
	Serial.print("\n");
	Serial.println("######################## BUFFER ENDE ############################");
	*/
...
uint8_t ATEMbase::getAudioSrcIndex(uint16_t audioSrc)	{
	switch(audioSrc){
		...
		case 2002:  // MP2
			return 24;
		case 3101:	// ATEM Mini TRS input 1
			return 25;
		case 3102:	// ATEM Mini TRS input 2
			return 26;
		default:
			return 0;
	}
...
uint16_t ATEMbase::getAudioIndexSrc(uint8_t index)	{
  switch (index) {
    ...
    case 24:  // MP2
      return 2002;
	case 25:	// ATEM Mini TRS input 1
		return 3101;
	case 26:	// ATEM Mini TRS input 2
		return 3102;
    default:
      return 0;
  }
